...
Binary outcomes -> the SOURCE betas are risk differences (BOLT-LMM linear
model on a 0/1 phenotype), not log-odds.  They are converted to the log-odds
scale with the BOLT-LMM manual transformation

        log OR = beta / (mu (1-mu)),    SE_logOR = SE / (mu (1-mu))

before any MR algebra, using the case fraction mu of the outcome GWAS.  Because
beta and SE are divided by the same constant, z statistics, P values, Cochran Q,
phi, the conditional F statistics and the tipping-point/ceiling ratio are all
invariant to this step; only odds ratios, confidence intervals, the minimum
detectable effect and the absolute size of the overlap bias change.

mu = 98704/461857 (knee), 118471/461857 (back) are taken from the OpenGWAS VCF
header.  The figshare replication source publishes no case count, so mu is
estimated from the standard errors: for a linear model on a 0/1 phenotype
SE^2 * 2 N f (1-f) = mu (1-mu), which recovers the two known values to within
0.2% and gives mu = 0.2236 (i.e. about 96,000 cases) here.
...
# The GIANT BMI coefficients are already per SD of BMI. The implied-SD
...
#
# lambda = 1/(mu (1-mu)) converts a BOLT-LMM risk difference into a log odds
# ratio.  mu for the two OpenGWAS sources comes from their VCF header; mu for
# the figshare replication source is estimated from the standard errors
# (see stage5_scale_audit_and_rescale.py, which reproduces all three values).
LAMBDA = {
    "BACK_PAIN_UKB": 5.24349,        # mu = 118471/461857 = 0.25651
    "KNEE_PAIN_UKB": 5.95101,        # mu =  98704/461857 = 0.21372
    "KNEE_PAIN_FIGSHARE": 5.76064,   # mu = 0.22358 (estimated, ~96,000 cases)
}
OUTCOMES = {
...
def main():
    ...
    for oc, path in OUTCOMES.items():
        ...
        lam = LAMBDA[oc]
        out["out_beta"] = out["out_beta"] * lam      # risk difference -> log OR
        out["out_se"] = out["out_se"] * lam
        ...
        # Frozen inputs shared with the independent base-R re-implementation are
        # written on the SOURCE (BOLT-LMM risk-difference) scale, so that each
        # implementation performs the documented log-odds conversion itself.
        # Scaling here instead would double-apply lambda inside the R script.
        d1_rinput = d1.copy()
        d1_rinput["out_beta"] = d1_rinput["out_beta"] / lam
        d1_rinput["out_se"] = d1_rinput["out_se"] / lam
        d1_rinput.to_csv(os.path.join(RIN, f"R_stage1_{oc}.tsv"), sep="\t", index=False)
        ...
        if loo:
            ...
            print(f"  LOO on BMI coefficient: min {loo[0][1]:+.5f} "
                  ...
                  f"full {m['theta'][1]:+.5f}")
        # Same convention as Stage 1 above: hand the base-R script the source
        # risk-difference inputs so it applies lambda exactly once.
        d2_rinput = d2.copy()
        d2_rinput["out_beta"] = d2_rinput["out_beta"] / lam
        d2_rinput["out_se"] = d2_rinput["out_se"] / lam
        d2_rinput.to_csv(os.path.join(RIN, f"R_stage2_{oc}.tsv"), sep="\t", index=False)
