...
SD_BMI   <- 1.0   # per SD of BMI (implied 1.23 reflects the release's ~26% SE inflation)

# The three outcome GWAS are BOLT-LMM linear-model analyses of a 0/1
# phenotype, so their coefficients are risk differences.  Convert to the
# log-odds scale with the BOLT-LMM manual transformation beta/(mu(1-mu));
# beta and SE share the divisor, so z, P, Q, phi and the Steiger test are
# unchanged.  Same constants as the Python pipeline (stage5_mr.py).
LAMBDA <- c(BACK_PAIN_UKB      = 5.24349,   # mu = 118471/461857
            KNEE_PAIN_UKB      = 5.95101,   # mu =  98704/461857
            KNEE_PAIN_FIGSHARE = 5.76064)   # mu = 0.22358 (estimated)
...
for (oc in OUTCOMES) {
  ...
  bY  <- d1$out_beta  * LAMBDA[[oc]]      # risk difference -> log odds
  seY <- d1$out_se    * LAMBDA[[oc]]
  cat("\n========", oc, "========\n")
  ...
  BY   <- d2$out_beta * LAMBDA[[oc]]       # risk difference -> log odds
  seBY <- d2$out_se   * LAMBDA[[oc]]
