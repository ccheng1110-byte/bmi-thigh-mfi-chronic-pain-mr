# =============================================================================
# Stage 6 - base-R replication of the FinnGen external-replication estimates
# -----------------------------------------------------------------------------
# Purpose: arithmetic cross-check of the Python pipeline (stage6_mr.py) using
#          ONLY base R - no TwoSampleMR / MVMR / MRPRESSO required.
...
#
# DIFFERENCE FROM STAGE 5 - READ THIS:
#   The Stage 5 outcomes were BOLT-LMM linear models on a 0/1 phenotype, so
#   their coefficients were risk differences and had to be multiplied by
#   lambda = 1/(mu(1-mu)) to reach log-odds.  FinnGen R12 fits REGENIE
#   logistic models, so the coefficients in r_input/ are ALREADY log odds
#   ratios.  There is no LAMBDA here and there must not be one: applying it
#   would be the Stage 5 double-conversion bug in the opposite direction.
#   See harmonise_finngen.py, which verified SE^2*2Nf(1-f) = 1/(mu(1-mu))
#   to within 1.6-4.5% on the non-significant instruments of all three
#   endpoints.
#
# Input : r_input/R_stage1_<OUTCOME>.tsv, r_input/R_stage2_<OUTCOME>.tsv
# Output: results/stage6_baser_all.csv
# =============================================================================

rin <- "D:/SX/TMFI_CPI_MR/06_stage6_finngen/r_input"
out <- "D:/SX/TMFI_CPI_MR/06_stage6_finngen/results"
dir.create(out, showWarnings = FALSE)

OUTCOMES <- c("FG_PAIN", "FG_LIMBPAIN", "FG_FIBROMYALGIA")
...
for (oc in OUTCOMES) {
  ...
  bY  <- d1$out_beta                     # already log-OR: NO lambda
  seY <- d1$out_se
  cat("\n========", oc, "========\n")
  ...
  BY   <- d2$out_beta                    # already log-OR: NO lambda
  seBY <- d2$out_se
...
write.csv(all_rows, file.path(out, "stage6_baser_all.csv"), row.names = FALSE)
cat("\n[done] wrote results/stage6_baser_all.csv\n")
print(all_rows)
