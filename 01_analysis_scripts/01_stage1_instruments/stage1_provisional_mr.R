...
run_mr <- function(path, outcome_name) {
  ...
  ivw_z <- ivw_beta / ivw_se_mre
  ivw_p <- 2 * pnorm(abs(ivw_z), lower.tail = FALSE)
...
full <- run_mr(file.path(base_dir, "cpi_full_distance10mb_harmonised.csv"), "CPI_full")
noimg <- run_mr(file.path(base_dir, "cpi_no_imaging_distance10mb_harmonised.csv"), "CPI_no_imaging")

methods <- rbind(full$methods, noimg$methods)
diagnostics <- rbind(full$diagnostics, noimg$diagnostics)

write.csv(methods, file.path(base_dir, "provisional_mr_methods.csv"), row.names = FALSE)
write.csv(diagnostics, file.path(base_dir, "provisional_mr_diagnostics.csv"), row.names = FALSE)
